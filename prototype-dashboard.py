    ...
    if name.endswith(".csv"):
        ...
        if "Period" in raw.columns and "Centre commercial Nice Valley" in raw.columns:
            ...
            df["Timestamp"] = pd.to_datetime(
                df["Period"] + "-1", format="%G-W%V-%u", errors="coerce"
            )
            df["Count"] = pd.to_numeric(
                df["Centre commercial Nice Valley"], errors="coerce"
            ).fillna(0)
        else:
...
c4.metric("Coût moyen/follower", f"{(df_monthly['Budget'].sum()/agg_f.sum()):.2f} EUR",
          help="Budget total dépensé divisé par le nombre de nouveaux abonnés.")

# --- KPI additionnels ---
st.markdown("### 2. KPI additionnels")
total_f    = agg_f.sum()
total_v    = agg_v.sum()
total_w    = agg_w.sum()
...
c5.metric("Conv. Followers/Vues", f"{conv_f:.2f}%", help="% de vues ayant généré un nouvel abonné.")
c6.metric("Conv. Visites/Vues",   f"{conv_v:.2f}%", help="% de vues ayant mené à une visite de profil.")
c7.metric("Croiss. mensuelle",    f"{avg_growth:.0f} foll/mois", help="Gain moyen de followers par mois.")
c8.metric("Mois de pic growth",   peak_month, help="Mois avec le plus fort gain de followers.")

# --- KPI Footfall ---
st.markdown("### 3. KPI Traffic physique")
...
# --- Data table ---
st.subheader("4. Données consolidées")
...
if st.button("Générer le rapport PDF"):
    ...
    # KPI additionnels
    pdf.set_font('Arial','B',14); pdf.cell(0,8,"KPI additionnels", ln=True)
    pdf.set_font('Arial','',11)
    ...
    pdf.set_font('Arial','',11)
    pdf.cell(0,6,f"Total Footfall      : {total_footall:,}", ln=True)  # attention à bien utiliser total_footfall
    pdf.ln(5)
